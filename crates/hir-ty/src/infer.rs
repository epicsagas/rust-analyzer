        ...
        match assoc_ty {
            Some(res_assoc_ty) => {
                let alias = Ty::new_alias(
                    ...
                    AliasTy::new(
                        self.interner(),
                        rustc_type_ir::AliasTyKind::Projection { def_id: res_assoc_ty.into() },
    ...
    fn resolve_variant(
        ...
        if let Some(type_anchor) = path.type_anchor() {
                    ...
                    .associated_type_by_name(segments.first().unwrap().name)
            {
                ...
                let alias = Ty::new_alias(
                    self.interner(),
                    AliasTy::new_from_args(self.interner(), rustc_type_ir::AliasTyKind::Projection { def_id: assoc_type.into() }, args),
                );
