...
impl<'db, 'a> TyLoweringContext<'db, 'a> {
            ...
            TypeRef::ImplTrait(bounds) => {
                match self.impl_trait_mode.mode {
                    ImplTraitLoweringMode::Opaque => {
                        ...
                        Ty::new_alias(
                            self.interner,
                            AliasTy::new_from_args(self.interner, rustc_type_ir::AliasTyKind::Opaque { def_id: opaque_ty_id }, args),
                        )
                    }
    ...
    fn lower_impl_trait(&mut self, def_id: SolverDefId, bounds: &[TypeBound]) -> ImplTrait {
        ...
        let self_ty = Ty::new_alias(
            self.interner,
            AliasTy::new_from_args(interner, rustc_type_ir::AliasTyKind::Opaque { def_id }, args),
        );
